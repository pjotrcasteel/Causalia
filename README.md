...
Deterministic simulation testing for .NET 10.

## Validation status

Verified on Linux on 2026-09-15 using the supplied .NET SDK **10.0.400** and runtime **10.0.11**, with no older runtime installed:

| Check | Result |
| --- | --- |
| Locked NuGet restore, vulnerability auditing enabled | Passed |
| Full solution, Release build | Passed; 0 warnings, 0 errors |
| Full test suite, .NET 10 | 224 passed, 0 failed, 0 skipped across 11 test projects |
| New storage/probability regression cases | 9 passed |
| Complete README implementation example | 3 tests passed |
| NuGet and symbol packages | 13 NuGet packages and 12 symbol packages verified |
| Isolated NuGet consumer | Restore, build, durable-storage and virtual-time execution passed |
| Packaged analyzer | Deliberate wall-clock access rejected with CAU1001 |
| Authored Markdown | Exactly one README in source and one README in each package |
| Windows CI | Configured; must run in the canonical GitHub repository before release |

Tests were verified without code coverage collection; the optional coverage collector did not start reliably in this execution environment.
The checked-in CI runs the complete tests with TRX output and a hang timeout. No tests are disabled or skipped to obtain this result.
This source revision removes older runtime targets, consolidates documentation, corrects post-write/delete ambiguous failures,
and rejects invalid `NaN` fault probabilities. The analyzer remains `netstandard2.0` for compiler-host compatibility;
its release-tracking files are generated under `obj` from `.txt` compiler metadata.

- [Implementation guide and complete example](#implementation-guide)
- [Build and release checks](#release-checks)

## Contents

- [Getting started and implementation](#overview)
- [AdvancedExploration](#advancedexploration)
- [Analyzers](#analyzers)
- [AspNetCore](#aspnetcore)
- [Consistency](#consistency)
- [EcosystemAdapters](#ecosystemadapters)
- [EntityFrameworkCore](#entityframeworkcore)
- [FailureIntelligence](#failureintelligence)
- [Features](#features)
- [Load](#load)
- [ModelBasedTesting](#modelbasedtesting)
- [Networking](#networking)
- [ProcessLifecycle](#processlifecycle)
- [ProductionReality](#productionreality)
- [Reqnroll](#reqnroll)
- [Storage](#storage)
...
`eng/verify.py` checks that there is one README, that its complete examples match source, and that runtime projects and packages target only .NET 10.
It verifies all 13 NuGet packages and 12 symbol packages. Its isolated consumer restores every package into a fresh cache, builds and runs a durable-storage
and virtual-time smoke test, then confirms the packaged analyzer rejects wall-clock access with CAU1001.
...
Contribution and security policies are sections of this README; GitHub may not display the dedicated policy tabs that it provides for separate policy files.
...
> Repository version: `2.0.1`. Runtime packages and tests target **net10.0 only**.
...
```xml
<ItemGroup>
  <PackageReference Include="Causalia" Version="2.0.1" />
  <PackageReference Include="Causalia.Analyzers" Version="2.0.1" />
</ItemGroup>
```
...
## Versioning and 2.0 platform stability
...
Causalia 2.0.1 keeps executable `v1:`, `m1:` and `mb1:` tokens unchanged. It emits collision-safe `fi2:` failure signatures and `pr2:` production
fingerprints; legacy `pr1` dataset JSON remains readable and is normalized to `pr2`. See [versioning policy](#versioning) and
[changelog](#changelog) for migration details.
...
```xml
<PackageReference Include="Causalia.Analyzers" Version="2.0.1" PrivateAssets="all" />
```
